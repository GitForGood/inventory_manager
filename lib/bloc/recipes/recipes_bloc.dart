...
class RecipesBloc extends Bloc<RecipesEvent, RecipesState> {
  ...
  Future<void> _onLoadAllRecipes(
    ...
    try {
      ...
      // Preserve favorites if they exist, otherwise load them
      List<Recipe> favorites = [];
      if (state is RecipesLoaded) {
        favorites = (state as RecipesLoaded).favorites;
      } else {
  ...
  Future<void> _onClearSearch(
    ...
    try {
      // Load favorites if state exists, otherwise fetch them
      List<Recipe> favorites = [];
      if (state is RecipesLoaded) {
        favorites = (state as RecipesLoaded).favorites;
      } else {
      ...
      emit(RecipesError('Failed to clear search: $e'));
      // Restore previous state if possible
      if (state is RecipesLoaded) {
        emit(state as RecipesLoaded);
      }
